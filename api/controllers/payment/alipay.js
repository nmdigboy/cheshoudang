...
module.exports = {
  ...
  fn: async function (inputs) {
    ...
    params.set('biz_content', _buildBizContent('宝马', '产品介绍', '201721546879', '0.01'));
    let Singed = await _buildSign(params);
    params.set('sign',Singed);
    ...
    let paramsString = paramsList.map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&');
    paramsString+='&sign='+encodeURIComponent(Singed);
